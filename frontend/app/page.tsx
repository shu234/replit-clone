...
export default function Page(){
  ...
  const loadFile = async (file: string) => {
    ...
    else if(file.endsWith('.go')) setLang('go');
    else setLang('node')
  }
  const saveFile = async () => {
    await fetch('https://replit-clone-i1ra.onrender.com/api/save',{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({code, file:activeFile})});
  ...
  const deploy = async () => {
    setOut('Deploying...');
    const res=await fetch('https://replit-clone-i1ra.onrender.com/api/deploy',{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({projectId:'demo-project', file:activeFile, code})});
    const d=await res.json();
    setDeployUrl(d.url);
    setShowDeployModal(true);
    setOut(`✅ DEPLOYED!\n${d.url}`);
    setPreviewUrl(d.localUrl);
    setShowPreview(true);
  }
  const install = async () => {
    if(!pkg) return;
    setOut(`Installing ${pkg}...`);
    const res=await fetch('https://replit-clone-i1ra.onrender.com/api/install',{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({pkg, lang})});
    const d=await res.json();
    setOut(d.stdout)
  }
