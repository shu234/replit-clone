    ...
    <div style={{display:'flex', height:'100vh', background:'#0e1525', color:'white', overflow:'hidden', fontFamily:'-apple-system, system-ui'}}>
      ...
      <div style={{flex:1, display:'flex', flexDirection:'column', minWidth:0}}>
        ...
        <div style={{display:'flex', flex:1, overflow:'hidden', minHeight:0}}>
          ...
          {showPreview && (<div style={{flex:'0 0 42%', display:'flex', flexDirection:'column', background:'white'}}><div style={{padding:'6px 10px', background:'#1c2333', color:'white', fontSize:'11px', display:'flex', justifyContent:'space-between'}}>
            
            <span>🌐 :4000</span><div style={{display:'flex', gap:'5px'}}><button onClick={()=>setPreviewUrl('https://replit-clone-i1ra.onrender.com?t='+Date.now())} style={{background:'#2b3245', color:'white', border:'none', padding:'3px 8px', borderRadius:'3px', fontSize:'10px'}}>↻</button><button onClick={()=>setShowPreview(false)} style={{background:'#ff4757', color:'white', border:'none', padding:'3px 8px', borderRadius:'3px', fontSize:'10px'}}>✕ Close</button></div>


          <iframe src={previewUrl} style={{flex:1, border:'none', background:'white', width:'100%'}} /></div>)}
        </div>
      </div>
